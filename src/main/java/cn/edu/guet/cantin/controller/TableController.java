...
public class TableController {
    ...
    /**
     * 更新餐桌信息接口（PUT请求）
     * @param tableInfo 前端传递的更新参数（需校验）
     * @return 统一响应结果
     */
    @PutMapping("/update") // 接口路径：/api/table/update，PUT方法
    public HttpResult updateTable(@RequestBody TableInfo tableInfo) {
        // 调用 Service 处理更新逻辑（异常会被全局异常处理器捕获）
        tableInfoService.updateTable(tableInfo);
        // 返回成功响应（Result 是自定义的统一响应类）
        return HttpResult.ok("餐桌更新成功");
    }
